...
for k, p in enumerate(cpuls[::2]):
    bscope[:, k] = np.mean(np.abs(p[:, :ngate] / 32e3) ** 2, axis=0)
bscope = 10.0 * np.log10(bscope)
m = [x[1] for x in cid]
n = cpuls[0].shape[0]
te = np.arange(int(naz / 2) + 1) * n * prt
re = 1.0e-3 * np.arange(ngate + 1) * dr
basename = os.path.basename(filename)
timestr = time.strftime('%Y%m%d-%H%M%S', scan_time)
folder = os.path.expanduser('~/Downloads/toshiba/figs')
...
plt.text(0.98 * te[-1], 0.95 * re[-1], basename, ha='right',
         fontsize=9, fontweight='bold', color='white', path_effects=path_effects)
fig.savefig('{}/bscope-{}.png'.format(folder, timestr))

def spectrogram(igate):
    nfft = 1024
    n = cpuls[0].shape[0]
    stride = slice(1, None, 2)
    tscope = np.zeros((len(az[stride]), nfft), dtype=np.csingle)
    w = scipy.signal.get_window('blackmanharris', n)
    w /= np.sqrt(np.mean(w ** 2))
    for k, cpul in enumerate(cpuls[stride]):
        p = cpul[:, igate] / 32000
        tscope[k, :] = sp.fft.fft(p * w, nfft, axis=0)
    tscope = sp.fft.fftshift(tscope, axes=(1,))
    tscope = 10.0 * np.log10(np.abs(tscope) ** 2 / np.sqrt(nfft))
    # Axes for plot
    te = np.arange(len(az[stride]) + 1) * stride.step * n * prt
    omega = np.arange(-nfft / 2, nfft / 2 + 1) / nfft
    # Figure
    fig = plt.figure()
    plt.pcolormesh(omega, te, tscope)
    plt.text(-0.48, 0.28, basename, ha='left',
         fontsize=9, fontweight='bold', color='white', path_effects=path_effects)
    plt.ylim((0, 7))
    plt.clim((-60, -10))
    cb = plt.colorbar()
    ...
    plt.ylabel('Time (s)')
    plt.title('Spectrogram of Cell #{} ({:.2f} km)'.format(igate, r[igate]))
    fig.savefig('{}/spg-{}-c{:03d}.png'.format(folder, timestr, igate))
